...
static int script_read_bootflow_file(struct udevice *bootstd,
  ...
  do {
    ...
    if (ret)
      ret = bootmeth_try_file(bflow, desc, prefix, SCRIPT_FNAME2);
  ...
  ret =
      bootmeth_alloc_other(bflow, "boot.bmp", &bflow->logo, &bflow->logo_size);
  /* ignore error */
...
static int script_read_bootflow_pciep(struct bootflow* bflow){
    const char *addr_str;
    ulong addr;
    int size;
    addr_str = env_get("scriptaddr");
    log_debug("scriptaddr: %s\n", addr_str);
    addr = addr_str ? hextoul(addr_str, NULL) : image_load_addr;
    size = env_get_hex("filesize", 0);
...
static int script_read_bootflow(struct udevice *dev, struct bootflow *bflow) {
  ...
  } else if(device_get_uclass_id(media) == UCLASS_PCI_EP){
      ret = script_read_bootflow_pciep(bflow);
      if(ret)
        return log_msg_ret("pciep", ret);
  }else {
...
static int script_boot(struct udevice *dev, struct bootflow *bflow) {
  ...
  if (bflow->blk) {
    ...
    if (desc->uclass_id == UCLASS_USB) {
       ...
       */
      if (IS_ENABLED(CONFIG_SATA) && desc->uclass_id == UCLASS_AHCI)
    ...
    ret = env_set("devtype", uclass_get_name(device_get_uclass_id(media)));
    if (!ret)
